...
def _sequential_fit_single_frame(xw, yw, peak_positions, anchor_peak0, baseline, noise,
    ...
    while n_added < min(_MAX_PEAKS, max(1, len(peak_positions))):
        ...
        # Use corresponding bounds for this guessed peak
...
def _run_tracking_up_to(h5_path, peak_positions, target_frame_inclusive, anchor_peak0=True,
    ...
    returning seeds and drift-bounded bounds for that target frame.
    Ensures --frame N uses identical seeding/bounds as the mapping pass.
    """
    with h5py.File(h5_path, "r") as f:
    ...
    for fr in range(tf):  # run up to frame tf-1 to prepare seeds/bounds for tf
        with h5py.File(h5_path, "r") as f:
            y = f["int"][fr, :]
    ...
    # ==== Seeds & bounds for target frame tf (ALIGNED) ====
    with h5py.File(h5_path, "r") as f:
        ytf = f["int"][tf, :]
    ...
    # seeds: track order, optionally append distinct anchor
    seeds_tf = list(map(float, track_centers))
    anchor_tf_val = float(anchor_centers[tf]) if (tf < len(anchor_centers) and np.isfinite(anchor_centers[tf])) else None
    def _is_close_to_any(val, arr, atol=1e-12):
        return np.any(np.isfinite(arr) & (np.abs(arr - val) <= atol))
    if anchor_tf_val is not None and not _is_close_to_any(anchor_tf_val, track_centers):
        seeds_tf.append(anchor_tf_val)

    # bounds: 1:1 with seeds (tracked → drift-bounded; anchor → generic)
    center_bounds_tf = []
    for idx, s in enumerate(seeds_tf):
        if idx < len(track_centers):
            grow = min(MISS_TOL_MAX, (DRIFT_TOL_GROW ** track_misses[idx]))
            cmin = max(xmin_tf, track_centers[idx] - DRIFT_TOL_NEG * grow)
            cmax = min(xmax_tf, track_centers[idx] + DRIFT_TOL_POS * grow)
            if cmin > cmax: cmin, cmax = cmax, cmin
        else:
            cmin = max(xmin_tf, s - CENTER_TOL_NEG)
            cmax = min(xmax_tf, s + CENTER_TOL_POS)
        center_bounds_tf.append((cmin, cmax))

    return {
        "track_centers": track_centers.copy(),
        "track_sigmas": track_sigmas.copy(),
        "center_bounds": center_bounds_tf,     # length == len(seeds_tf)
        "merged_seeds": seeds_tf,              # ordered: tracks then (maybe) anchor
        "target_frame": tf
    }

# ==============================
# Map across frames (uses same tracking; seeds/bounds aligned)
# ==============================
...
def map_peaks_over_frames(h5_path, peak_positions, *, anchor_peak0=ANCHOR_PEAK0):
    ...
    for fr in range(nframes):
        ...
        # --- ordered seeds: tracks then (maybe) anchor, keeping them distinct ---
        merged_seeds = list(map(float, track_centers))
        anc = float(anchor_centers[fr]) if np.isfinite(anchor_centers[fr]) else None
        def _close(val, arr, atol=1e-12):
            return np.any(np.isfinite(arr) & (np.abs(arr - val) <= atol))
        if anc is not None and not _close(anc, track_centers):
            merged_seeds.append(anc)

        # --- bounds aligned 1:1 with seeds ---
        center_bounds = []
        for k, s in enumerate(merged_seeds):
            if k < len(track_centers):
                grow = min(MISS_TOL_MAX, (DRIFT_TOL_GROW ** track_misses[k]))
                cmin = max(xmin, track_centers[k] - DRIFT_TOL_NEG * grow)
                cmax = min(xmax, track_centers[k] + DRIFT_TOL_POS * grow)
                if cmin > cmax: cmin, cmax = cmax, cmin
            else:
                cmin = max(xmin, s - CENTER_TOL_NEG)
                cmax = min(xmax, s + CENTER_TOL_POS)
            center_bounds.append((cmin, cmax))

        # Fit with aligned seeds/bounds
...
def main():
    ...
    if args.frame is not None:
        # Ensure single-frame uses identical tracking up to that frame
